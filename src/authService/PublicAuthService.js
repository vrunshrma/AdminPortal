...
const AUTH_REST_API_BASE_URL = 'http://localhost:9090/api/v1/auth';
...
export const passwordCreationEmail = async (email) => {
  try {
    const response = await axios.get(`${AUTH_REST_API_BASE_URL}/createPassword?email=${email}`);
    return response.data;
  } catch (error) {
    if (error.response) {
      return error.response.data;
...
export const validateToken = (userId, passwordResetToken) =>
  axios.get(`${AUTH_REST_API_BASE_URL}/validateToken`, {
    params: {
      id: userId,
